import { GearApi, getProgramMetadata, MessageEnqueued } from '@gear-js/api';
...
export async function uploadProgram(
  ...
  const [blockHash, msgId]: [`0x${string}`, `0x${string}`] = await new Promise((resolve) =>
    extrinsic.signAndSend(account, ({ events, status }) => {
      const meEvent = events.find(({ event: { method } }) => method === 'MessageEnqueued');
      if (meEvent) {
        if (status.isInBlock) {
          resolve([status.asInBlock.toHex(), (meEvent.event as MessageEnqueued).data.id.toHex()]);
        }
      }
    }),
  );
